...
namespace pr{
    ...
    public:
        ...
        static cv::Mat FineMappingVertical(cv::Mat InputProposal,int sliceNum=15,
                                           int upper=0,int lower=-50,int windows_size=17);

        cv::Mat FineMappingHorizon(cv::Mat FinedVertical,int leftPadding,int rightPadding);

        static std::vector<cv::Point> get4point(cv::Mat InputProposal,
                                                int sliceNum = 15,
                                                int upper = 0,
                                                int lower = -50, int windows_size = 17);

        static std::vector<cv::Point> get4point2(cv::Mat InputProposal,
                                                 int sliceNum = 15,
                                                 int upper = 0,
                                                 int lower = -50, int windows_size = 17);


        static std::vector<cv::Rect> get7Rect(cv::Mat InputProposal,
                                              int sliceNum = 15,
                                              int upper = 0,
                                              int lower = -50, int windows_size = 17);
