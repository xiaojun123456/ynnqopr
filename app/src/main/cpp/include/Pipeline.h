...
namespace pr{
    ...
    class PipelinePR{
        ...
        //e2e
        std::vector<cv::Point> GetPointFromPlateRough(cv::Mat plateImage);

        //c2c
        std::pair<std::string,std::vector<cv::Point>> GetPointFromPlateRough2(cv::Mat plateImage);

        std::pair<std::string,float> GetPlateResult(cv::Mat plateImage,std::vector<cv::Point> cors);
